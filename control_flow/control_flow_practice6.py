...
def create_user(user_data):
    ...
    for data in user_data:    
        ...
        elif is_validation(data)[0] == False:
            number += 1
            for wrong_data in is_validation(data)[1]:
                data[wrong_data] = None
        user_list.append(data)       
    print(f'잘못된 데이터로 구성된 유저의 수는 {number} 입니다.')   
    return user_list
...
# def is_validation(data):
#     if (data['blood_group'] in blood_types) and (data['company'] not in black_list) and ('@' in data['mail']) and 2 <= len(data['name']) <= 30 and len(data['website']) >= 1:
#         return True
#     else:
#         if data['company'] in black_list:       
#             return 'blocked'
