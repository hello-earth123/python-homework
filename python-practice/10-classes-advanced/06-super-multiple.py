...
class ParentA:
    def __init__(self):
        # parentA에서의 super는 MRO 순서에 따른 클래스를 호출한다. (단순히 부모 클래스를 호출하는 것이 아니라)
        # ParentA에서 super를 한다면 MRO 순서에 따라 A->B순서에서의해 B를 호출한다.
        # ParentA가 상속을 받아도 상속된 클래스는 B 다음으로 온다. -> MRO의 시작을 기준으로 한다.
        super().__init__() 
